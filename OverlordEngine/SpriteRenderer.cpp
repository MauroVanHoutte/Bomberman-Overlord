...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if (m_pVertexBuffer == nullptr || m_Sprites.size() > m_BufferSize)
	{
		...
		if (FAILED(gameContext.pDevice->CreateBuffer(&bd, nullptr, &m_pVertexBuffer)))
		{
			Logger::LogError(L"Spriterenderer Buffer Creation failed", true);
		}
	}
	...
	memcpy(subresource.pData, m_Sprites.data(), sizeof(SpriteVertex) * m_BufferSize);
